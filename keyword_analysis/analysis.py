...
def wordcloud_(wxid, time_range=None):
    ...
    stopwords_file = os.path.join(current_dir, '.', 'resources', 'stopwords.txt')
    with open(stopwords_file, "r", encoding="utf-8") as stopword_file:
...
def get_wordcloud(text):
    total_msg_len = len(text)
    current_dir = os.path.dirname(__file__)
    newword_file = os.path.join(current_dir, '.', 'data', 'new_words.txt')
    jieba.load_userdict(newword_file)
    # 使用jieba进行分词，并加入停用词
    words = jieba.cut(text)
    # 统计词频
    word_count = Counter(words)
    ...
    w.render("./data/聊天统计/wordcloud.html")
    # w.render_notebook()
    return {
...
if __name__ == '__main__':
    msg_db.init_database(path='./data/MSG.db')
    wxid = 'wxid_ft116aa2ff7522'
    w_data = wordcloud_christmas(wxid)
    # w_data = wordcloud_(wxid, True, '2023')
    # print(w_data)
